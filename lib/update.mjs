...
import { dashboardStatus, restartDashboard } from './autostart.mjs'

export async function reconcileDashboardUpdate({ dryRun = false, dashboardOptions = {} } = {}, { status = dashboardStatus, restart = restartDashboard } = {}) {
  const before = await status(dashboardOptions)
  if (!before.enabled) return { ok: true, action: before.disabled ? 'disabled' : 'none', status: before }
