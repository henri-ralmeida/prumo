...
export async function launchUpdate(request, { env = process.env } = {}) {
  if (request.updateCli === false && !discoverInstallations({ cwd: request.cwd, projects: request.projects, env }).length) return null
  if (request.sourceVersion) assertUpdateVersion(request.sourceVersion, npmLatestVersion({ env }))
