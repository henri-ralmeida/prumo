...
export function installationsCurrent(installations, version, { read = readFileSync } = {}) {
  return installations.every(entry => entry.roots.every(root => {
    try { return JSON.parse(read(join(root, 'prumo', '.prumo-install.json'), 'utf8').replace(/^\uFEFF/, '')).version === version }
    catch { return false }
  }))
}
...
export async function launchUpdate(request, { env = process.env, onCurrent = () => {}, discover = discoverInstallations, latestVersion = npmLatestVersion, run = spawn } = {}) {
  ...
  if (request.sourceVersion) {
    ...
    const globalVersion = request.globalVersion ?? request.sourceVersion
    if (request.lang === undefined && globalVersion === latest && installationsCurrent(installed, latest)) {
      onCurrent(latest)
      return 0
    }
  }
