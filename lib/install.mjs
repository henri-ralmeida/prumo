import { existsSync, readFileSync, writeFileSync, mkdirSync, readdirSync, lstatSync, renameSync, unlinkSync, cpSync, statSync, accessSync, constants } from 'node:fs'
  ...
  for (const root of targets) {
    ...
    group(`skill:${root}`, [destination, legacy], item => {
      ...
      if (existsSync(legacy)) {
        ...
        change(item, join(legacy, 'scripts', 'dashboard.html'), localizeDashboard(readFileSync(join(PACKAGE, 'scripts', 'dashboard.html'), 'utf8'), lang))
