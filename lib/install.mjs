  ...
  if (harness === 'claude') {
    ...
    if (existsSync(settings)) {
      ...
      const inspectSkill = () => {
        ...
        const body = skillBlock.join('\n')
        const path = body.match(/^\s*path\s*=\s*(["'])(.*?)\1\s*(?:#.*)?$/m)?.[2]
        if (path && /(?:^|[\\/])(?:prumo|graph-foreman)(?:[\\/]|$)/i.test(path) &&
            /^\s*enabled\s*=\s*false\s*(?:#.*)?$/m.test(body)) {
          result.warnings.push(`Skill explicitly disabled; activation needs a settings change: ${settings}`)
        }
      }
      for (const line of readFileSync(settings, 'utf8').split(/\r?\n/)) {
        if (/^\s*\[/.test(line)) {
          inspectSkill()
          skillBlock = /^\s*\[\[skills\.config\]\]\s*(?:#.*)?$/.test(line) ? [] : null
        } else if (skillBlock) skillBlock.push(line)
      }
      inspectSkill()
    }
  } else if (harness === 'dsh') {
