...
export function planInstall({ harness, home = homedir(), cwd = process.cwd(), env = process.env, lang, projects = [] }) {
  if (!['claude', 'kiro', 'codex'].includes(harness)) throw new Error('Choose exactly one of --claude, --kiro or --codex')
  home = resolve(home)
  const config = harness === 'claude' ? resolve(env.CLAUDE_CONFIG_DIR ?? join(home, '.claude')) : harness === 'codex' ? resolve(env.CODEX_HOME ?? join(home, '.codex')) : join(home, '.kiro')
  ...
  const targets = [...new Set([globalSkills, ...existing])]
