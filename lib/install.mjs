...
function bundle(lang) {
  ...
  for (const name of ['messages.json', 'region.mjs', 'i18n.mjs', 'storage.mjs', 'validation.mjs', 'dashboard.html', 'engine.mjs', 'serve.mjs']) files.push(join('scripts', name))
  files.push('SKILL.md')
...
export function detectHarnesses({ home = homedir(), cwd = process.cwd(), env = process.env, projects = [] } = {}) {
  ...
  return ['claude', 'kiro', 'codex'].filter(harness => {
    if (directory(configFor(harness, home, env))) return true
    if ((harness === 'kiro' ? ['kiro-cli', 'kiro'] : [harness]).some(executable)) return true
    // Shared .agents directories alone do not identify an installed harness.
    return harness !== 'codex' && search.some(root => ['graph-foreman', 'prumo'].some(skill => existsSync(join(root, `.${harness}`, 'skills', skill, 'SKILL.md'))))
  })
}
