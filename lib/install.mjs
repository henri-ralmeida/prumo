...
import { language, localizeDashboard } from '../scripts/i18n.mjs'
...
function copyRelocatedTree(current, relocated, backupRoot, originalRoot, destinationRoot, links = []) {
  ...
  if (stat.isSymbolicLink()) {
    ...
    const target = relocatedTarget(savedTarget, originalRoot, destinationRoot, original, relocated)
    links.push({ target, relocated, type: targetStat?.isDirectory() ? 'junction' : 'file' })
  } else if (stat.isDirectory()) {
    mkdirSync(relocated, { recursive: true })
    for (const name of readdirSync(current)) copyRelocatedTree(join(current, name), join(relocated, name), backupRoot, originalRoot, destinationRoot, links)
  } else {
...
function verifyRelocatedTree(backupRoot, destinationRoot, originalRoot) {
  ...
  return before.every(name => {
    ...
    if (stat.isSymbolicLink()) {
      ...
      return linkTarget(destination) === relocatedTarget(linkTarget(source), originalRoot, destinationRoot, original, destination)
    }
...
function bundle(lang) {
  ...
  for (const name of ['messages.json', 'region.mjs', 'i18n.mjs', 'storage.mjs', 'atomic-state.mjs', 'validation.mjs', 'dashboard.html', 'engine.mjs', 'serve.mjs']) files.push(join('scripts', name))
  files.push('SKILL.md')
  return files.map(name => [name, name === join('scripts', 'dashboard.html') ? Buffer.from(localizeDashboard(readFileSync(join(PACKAGE, name), 'utf8'), lang)) : readFileSync(join(PACKAGE, name))])
}
...
const configFor = (harness, home, env) => harness === 'claude' ? resolve(env.CLAUDE_CONFIG_DIR || join(home, '.claude')) : harness === 'codex' ? resolve(env.CODEX_HOME || join(home, '.codex')) : resolve(env.KIRO_HOME || join(home, '.kiro'))
...
export function detectHarnesses({ home = homedir(), cwd = process.cwd(), env = process.env, projects = [] } = {}) {
  ...
  return ['claude', 'kiro', 'codex'].filter(harness => {
...
export async function reconcileDashboardInstall(successfulHarnesses, { dryRun = false, dashboardOptions = {}, status = dashboardStatus, enable = enableDashboard, restart = restartDashboard } = {}) {
  if (successfulHarnesses < 1) return { ok: false, action: 'none' }
  const before = await status(dashboardOptions)
  ...
  const result = await (action === 'restart' ? restart : enable)(dashboardOptions)
...
export function discoverInstallations({ home = homedir(), cwd = process.cwd(), env = process.env, projects = [], onError = error => { throw error } } = {}) {
  ...
  return entries.map(entry => ({ ...entry, projects: [...new Set(entry.projects)], roots: [...new Set(entry.roots)].filter(root => {
    ...
    if (entry.harness === 'codex' && root === legacyCodex && root !== sharedCodex && managed(sharedCodex, 'codex')) return false
    ...
    if (existsSync(file) || registeredRoots.has(root) && existsSync(join(root, 'prumo'))) {
      try {
        ...
        if (!marker || marker.product !== 'prumo' || !['claude', 'kiro', 'codex'].includes(marker.harness) || typeof marker.version !== 'string') throw new Error('invalid metadata')
      } catch (error) {
        invalid.add(root)
    ...
    if (!managed(root, entry.harness)) return false
    claimed.add(root)
    return true
...
export function planInstall({ harness, home = homedir(), cwd = process.cwd(), env = process.env, lang, projects = [], configRoot, skillRoots, onlyInstalled = false }) {
  ...
  const targets = [...new Set(skillRoots ?? [primarySkills, ...existing])].filter(root => !onlyInstalled || managed(root, harness))
  ...
  if (!sameHome && existsSync(legacyHome)) for (const entry of readdirSync(legacyHome, { withFileTypes: true })) {
    const source = join(legacyHome, entry.name)
    if (!entry.isDirectory() || !existsSync(join(source, '.specs', 'graph'))) continue
    const destination = join(prumoHome, entry.name)
    result.allowedRoots.push(source, destination)
    const migration = group(`workspace:${entry.name}`, existsSync(destination) ? [source, destination] : [], item => {
      noLinks(source)
      noLinks(destination)
      if (!existsSync(destination)) {
        item.relocation = { source, destination, stamp: treeStamp(source) }
        return
      }
      const destinationFiles = new Set(walk(destination, destination, true).map(name => process.platform === 'win32' ? name.toLowerCase() : name))
      const sourceFiles = walk(source, source, true)
      ...
      for (const name of sourceFiles) {
        ...
        if (link) {
          ...
          if (isAbsolute(targetPath) && inside(source, absolute)) content = Buffer.from(join(destination, relative(source, absolute)))
          else if (!isAbsolute(targetPath) && !inside(source, absolute)) content = Buffer.from(relative(dirname(target), absolute))
        }
      ...
      item.cleanup.push({ path: source, optionalWhenEmpty: true })
    })
    if (!migration.changes.length && !migration.relocation && !migration.conflicts.length) continue
    result.data = result.data.filter(data => !(inside(data.root, destination) && inside(destination, data.root)))
    result.data.push({ root: destination, source, action: 'migrate' })
  }
  ...
  for (const group of plan.groups) {
    ...
    try {
      if (group.relocation) {
        ...
        try {
          copyRelocatedTree(saved, entry.destination, saved, entry.source, entry.destination)
          if (!verifyRelocatedTree(saved, entry.destination, entry.source)) throw new Error(`Workspace integrity check failed: ${entry.destination}`)
...
export function restoreInstall(backup, { home = homedir(), env = process.env } = {}) {
  ...
  const pendingRelocations = relocations.filter(entry => {
    ...
    if (!existsSync(saved) || !existsSync(entry.destination) || !verifyRelocatedTree(saved, entry.destination, entry.source)) throw new Error(`Workspace changed since this backup; restore manually: ${entry.destination}`)
