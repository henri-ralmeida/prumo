import { existsSync, readFileSync, writeFileSync, mkdirSync, readdirSync, lstatSync, renameSync, unlinkSync, rmdirSync, copyFileSync, statSync, accessSync, constants, readlinkSync, symlinkSync, realpathSync } from 'node:fs'
...
export function planInstall({ harness, home = homedir(), cwd = process.cwd(), env = process.env, lang, projects = [], configRoot, skillRoots, onlyInstalled = false }) {
  ...
  if (!sameHome && existsSync(legacyHome)) for (const entry of readdirSync(legacyHome, { withFileTypes: true })) {
    ...
    const migration = group(`workspace:${entry.name}`, [source, destination], item => {
      noLinks(source)
      noLinks(destination)
    ...
    if (!migration.changes.length && !migration.conflicts.length) continue
...
export function applyInstall(plan, { dryRun = false } = {}) {
  ...
  const actionable = plan.groups.filter(group => !group.conflicts.length && (group.changes.length || group.cleanup.length))
  let backup = null
  ...
  for (const group of plan.groups) {
    ...
    try {
      ...
      results.push({ name: group.name, status: group.changes.length || group.cleanup.length ? 'installed' : 'unchanged', errors: [] })
    } catch (error) {
