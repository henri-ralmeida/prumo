...
function bundle(lang) {
  ...
  for (const name of ['messages.json', 'region.mjs', 'i18n.mjs', 'storage.mjs', 'atomic-state.mjs', 'validation.mjs', 'sync-plan-audit.mjs', 'dashboard-diagnostics.mjs', 'dashboard.html', 'engine.mjs', 'serve.mjs']) files.push(join('scripts', name))
  files.push('SKILL.md')
...
export function readInstallationMarker(root, { home = homedir(), harness } = {}) {
  ...
  try {
    ...
    if (!marker || marker.product !== 'prumo' || !['claude', 'kiro', 'codex'].includes(marker.harness) || !/^\d+\.\d+\.\d+$/.test(marker.version)) throw new Error('invalid metadata')
    return marker
  } catch (error) {
...
export function installationFilesCurrent(root, marker) {
  try {
    ...
    noLinks(destination)
    return bundle(marker.lang).every(([name, expected]) => {
