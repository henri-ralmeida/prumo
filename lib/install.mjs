import { existsSync, readFileSync, writeFileSync, mkdirSync, readdirSync, lstatSync, renameSync, unlinkSync, rmdirSync, cpSync, statSync, accessSync, constants } from 'node:fs'
...
export function planInstall({ harness, home = homedir(), cwd = process.cwd(), env = process.env, lang, projects = [], configRoot, skillRoots, onlyInstalled = false }) {
  ...
  if (!sameHome && existsSync(legacyHome)) for (const entry of readdirSync(legacyHome, { withFileTypes: true })) {
    ...
    const migration = group(`workspace:${entry.name}`, [source, destination], item => {
      const destinationFiles = new Set(walk(destination).map(name => process.platform === 'win32' ? name.toLowerCase() : name))
      ...
      for (const name of sourceFiles) {
        ...
        if (destinationFiles.has(key) && !same(bytesAt(target), content)) throw new Error(`Legacy workspace conflicts with Prumo destination: ${target}`)
        change(item, target, content)
      }
      for (const name of sourceFiles) change(item, join(source, name), null)
