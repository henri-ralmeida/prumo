...
function defaults(options = {}) {
  ...
  const lang = options.lang ?? language(undefined, options.env ?? process.env)
  const identity = options.contentId ? { contentId: options.contentId, engineHash: options.engineHash ?? null } : packageIdentity(lang, { packageRoot })
  if (![node, script].every(isAbsolute)) throw new Error('Dashboard autostart requires absolute Node and package paths')
  return {
    platform, home, node, script, version, lang, packageRoot, ...identity,
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (health.product === 'prumo' && health.mode === 'global' && health.readOnly === true) {
        ...
        } catch { /* older dashboards expose health but not package identity */ }
...
export async function restartDashboard(options = {}) {
  ...
  if (entry.mechanism === 'schtasks') {
    ...
    if (created.status === 0) {
      if (!present && health.state === 'running') {
        const pid = fallbackPid(o, preference, health)
        if (!pid) return { ok: false, error: 'Startup ownership verification failed: managed process not found', ...(await dashboardStatus(o)) }
        try { o.kill(pid) }
