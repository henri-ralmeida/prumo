...
function windowsProcessCommand(pid) {
  ...
  return JSON.parse(execFileSync('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command', source], { encoding: 'utf8', windowsHide: true }))
}

function windowsProcessIds() {
  const source = `Get-CimInstance Win32_Process -Filter "Name = 'node.exe'" | Select-Object -ExpandProperty ProcessId`
  return execFileSync('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command', source], { encoding: 'utf8', windowsHide: true })
...
function defaults(options = {}) {
  ...
  return {
    ...
    exec: options.exec ?? ((file, args, extra = {}) => {
      try { return { status: 0, stdout: execFileSync(file, args, { encoding: 'utf8', windowsHide: true, ...extra }) } }
    ...
    readinessAttempts: options.readinessAttempts ?? 10,
...
function launchDetached(o) {
  ...
  return o.spawn(o.node, command(o.node, o.script).slice(1), { cwd, detached: true, stdio: 'ignore', windowsHide: true })
}
...
export async function enableDashboard(options = {}) {
  ...
  if (entry.mechanism === 'launchd') {
    ...
    if (health.state !== 'running') {
      try { pid = launchDetached(o).pid ?? null }
      catch (error) {
        writePreference(o, { enabled: true, mechanism: entry.mechanism, node: o.node, script: o.script })
    ...
    const resolvedPid = ready.state === 'running'
      ? fallbackPid(o, { ...preference, pid, node: o.node, script: o.script }, ready)
      : pid
...
function managedFallbackPid(o, pid, preference) {
  try {
    ...
    const quoted = expected.map(quoteWindows).join(' ')
    const minimal = expected.map(value => /[\s"]/.test(value) ? quoteWindows(value) : value).join(' ')
    const quotedExecutable = [quoteWindows(expected[0]), ...expected.slice(1).map(value => /[\s"]/.test(value) ? quoteWindows(value) : value)].join(' ')
    return [quoted, minimal, quotedExecutable].includes(cmdline.commandLine)
  } catch { return false }
}
...
function fallbackPid(o, preference, health) {
  ...
  let pids = []
  try { pids = o.listProcessIds() } catch {}
