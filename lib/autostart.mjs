...
function windowsProcessIds(script) {
  ...
  const source = `Get-CimInstance Win32_Process -Filter "Name = 'node.exe'" | Where-Object { $_.CommandLine -and $_.CommandLine.Contains('${literal}') } | Select-Object -ExpandProperty ProcessId`
...
function managedFallbackPid(o, pid, preference) {
  try {
    ...
    const normalized = String(cmdline.commandLine ?? '').replace(/"[^"]*"|[ \t]+/g,
      token => token.startsWith('"') ? token : ' ').trim()
    return [quoted, minimal, quotedExecutable].includes(normalized)
  } catch { return false }
}
