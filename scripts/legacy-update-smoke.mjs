...
// Exercise the actual published 1.0.8 updater against a local registry serving the candidate.
...
try {
  ...
  run('npm', ['install', '--global', '--ignore-scripts', '--no-audit', '--no-fund', join(repo, 'henri-ralmeida-prumo-1.0.8.tgz')])
  assert.match(run(process.execPath, [cli, '-v']), /1\.0\.8/)
  run(process.execPath, [cli, 'install', '--all', '--lang', 'en'])
  ...
  assert.match(output, /Prumo v1\.0\.9/)
  assert.match(output, /Prumo v1\.3\.4/)
  ...
  console.log(`Published 1.0.8 updated to ${pkg.version}; complete notes, three harnesses, legacy data relocation and merged-task migration verified`)
} finally {
  ...
  put(join(repo, '.test-output/legacy-update-smoke.json'), evidence)
