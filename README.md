...
`prumo status` lists the harness, version, language and content identifier for every registered installation. It also warns when an older marker has no identity, when the `engine.mjs` hash diverges, or when installed content differs from this package. `prumo status --verify-install` checks every distributed file. `doctor` compares registered installations in the same language, including DSH. Before publishing a change to a distributed file, bump its SemVer version; use a pre-release only after updating SemVer validation, which currently accepts only `X.Y.Z`. `npm run check` and `npm pack` also compare the distributed file manifest to `scripts/release-baseline.json`; it records the published 1.3.16 npm tarball and its SHA-512 integrity. Refresh that baseline only from the exact package contents after a release is published.
