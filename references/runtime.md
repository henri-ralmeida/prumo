...
New 1.2.1 tasks, including tasks added by `sync-plan` to old runs, receive
`discoveryRequired: true` and `planningRequired: true`.
...
Planning has two levels. Global Plan/Spec mode defines and approves the graph. During execution,
each task follows **discuss → plan → executor → reviewer**; the run need not stay in global Plan
mode. After dependencies are `done` or `skipped`, the principal conversation loads that global
context and dependency outputs, scouts the task and asks at least one contextual question. It
...
```json
{
  ...
  "questions": [{ "question": "Should invalid input reject the whole import?", "answer": "Yes; no partial writes.", "channel": "chat-fallback", "round": 1 }],
  "coverage": {
...
`plan-task T1 --agent <planner> --context <discovery.json>` requires nonempty light research,
...
reason. It validates everything before atomically persisting discovery and entering `planning`.
The engine computes a canonical SHA-256 digest from exactly those persisted fields and binds it to
the planning round. Resubmitting identical content while planning changes no state and records no
new round; changed content closes the open round as `superseded` and opens another bound to the new
digest. `finish-planning` copies that digest to the taskPlan and refuses stale or altered discovery.
The dedicated planner then researches the **current** code, artifacts and dependency outputs in
depth, consumes the persisted decisions without asking them again, and writes its designated
task-plan JSON. New material uncertainty returns to principal discovery and fresh planning.
...
node .claude/skills/prumo/scripts/engine.mjs plan-task T1 --agent plan-server --context <discovery.json>  # after discuss; dispatch native planner
...
- `plan-task` and `start` require complete dependencies; new 1.2.1 tasks require a valid discovery
  context before `plan-task`, and new tasks also require current planning
