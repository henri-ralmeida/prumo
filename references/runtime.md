...
one separately bound immutable `task-plan-<id>.json` per targeted task. Discussion and planning may run
before member dependencies complete; the task DAG remains the sole execution authority. The conversation
...
Runs created by an older engine keep their task commands unchanged. They can adopt a whole phase with
`begin-phase-discussion F1 --adopt-legacy`. Terminal history is untouched; every adopted nonterminal member
must be pre-execution and have no open task discussion or planning round. Unsafe adoption changes nothing.
...
Task `pending` is stored; task readiness is derived from dependencies and current planning. The phase
workflow persists `ready_for_discussion`, `discussing`, `ready_to_plan` and `planning` independently.
Discussion and planning may therefore be active while a member card remains `waiting` for task inputs.
