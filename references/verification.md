...
|---|---|
| 1.0.6 automated suite / Suíte automatizada 1.0.6 | Engine, installer, update and dashboard checks run in CI; the release commit's Actions result is the source of truth / Verificações de motor, instalador, atualização e dashboard rodam no CI; o resultado do Actions para o commit da release é a fonte de verdade |
...
| Public update command / Comando público de atualização | `scripts/package-smoke.mjs` uses an isolated npm prefix and registry to verify persistent CLI installation, `prumo -v`, dry-run, self-update, all three adapters and registry failure / O script usa prefixo e registro npm isolados para verificar instalação persistente da CLI, `prumo -v`, prévia, autoatualização, os três adaptadores e falha do registro |
...
| Local Claude Code, Kiro, Codex / Ambientes locais | 1.0.5 adapter files were observed before this release; public 1.0.6 self-update remains a maintainer acceptance test / Arquivos dos adaptadores 1.0.5 foram observados antes desta release; a autoatualização pública 1.0.6 continua como teste de aceitação do mantenedor |
