...
import { updateRequest } from '../lib/update.mjs'
...
test('update detects installed harnesses and custom paths, preserves preferences, previews and skips legacy-only environments', t => {
  ...
  const run = dryRun => spawnSync(process.execPath, [join(source, 'bin', 'prumo.mjs'), '_update'], { cwd, env: { ...env, PRUMO_UPDATE_REQUEST: JSON.stringify({ dryRun, cwd, projects: [] }) }, encoding: 'utf8', timeout: 120000, windowsHide: true })
