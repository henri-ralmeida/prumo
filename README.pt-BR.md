...
`prumo status` mostra harness, versão, idioma e identificador do conteúdo de cada instalação registrada. Também avisa quando um marcador antigo não guarda a identidade, quando o hash de `engine.mjs` diverge ou quando o conteúdo instalado difere deste pacote. `prumo status --verify-install` compara todos os arquivos distribuídos. `doctor` compara o conteúdo das instalações registradas no mesmo idioma, incluindo DSH. Antes de publicar uma mudança em arquivo distribuído, incremente a versão SemVer; use uma pré-versão somente depois de ajustar a validação SemVer, que atualmente aceita apenas `X.Y.Z`. `npm run check` e `npm pack` também comparam o manifesto dos arquivos distribuídos com `scripts/release-baseline.json`; ele registra o tarball 1.3.16 publicado no npm e sua integridade SHA-512. Atualize essa referência somente com o conteúdo exato do pacote depois de uma publicação.
